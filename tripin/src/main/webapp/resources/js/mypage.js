$(function(){
	...
	$('#content-saved').on('click', '.detail-button', function(){
		...
		let dest_id = $(this).parents('.place-info').find('#dest_id').val();
		
		// 상세정보에서 url 어떻게 받는지 확인해야됨
		location.href = `detail/dest_id=${dest_id}.do`
	...
	$('#content-saved').on('click', '.heart-button', function(){
	    ...
	    if(heartIcon.hasClass('ri-heart-line')) {
	    	dest_id = heartIcon.parents('.dest_id');
	    	let param = {dest_id:dest_id.val()};
			$.ajax({
				type:'delete'
				, data: param
				, url: 'deleteplace'
				, success: function(){
					dest_id.parents('.place-card').remove();
